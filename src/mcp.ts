...
export async function getMcpProxyClient(command: string[]): Promise<Client> {
    ...
    // StdioClientTransport only inherits a minimal, fixed set of env vars by default
    // (see DEFAULT_INHERITED_ENV_VARS in the SDK). Pass through the full parent env so that
    // Apify platform vars (APIFY_TOKEN, ACTOR_RUN_ID, APIFY_IS_AT_HOME, ...) reach the proxied
    // MCP server process - required for it to call Actor.charge() correctly.
