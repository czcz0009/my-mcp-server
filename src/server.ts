 ...
 * It manages session-based transports and request routing.
 * Billing is handled per-tool inside the proxied MCP server (see pkg-health/mcpServer.js),
 * not at the protocol level here.
 *
...
export async function startServer(options: { serverPort: number; command: string[] }) {
    ...
    // Default express.json() limit (100kb) is too small for tools that take a whole
    // package-lock.json as input (check_dependency_tree_risk) - large projects can run
    // into the low single-digit MBs.
